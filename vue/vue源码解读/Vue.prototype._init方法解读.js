...
export function initMixin (Vue: Class<Component>) {
  ...
  Vue.prototype._init = function (options?: Object) {
    ...
    initEvents(vm)
    initRender(vm)
    callHook(vm, 'beforeCreate')// 抛出beforeCreate钩子函数
    initInjections(vm) // resolve injections before data/props 没用过吗，先不管
    initState(vm) // 对data，props，methods，computed, watch的操作
    initProvide(vm) // resolve provide after data/props 没用过吗，先不管
    callHook(vm, 'created')// 抛出created钩子函数
...
export function resolveConstructorOptions (Ctor: Class<Component>) {
    let options = Ctor.options // Vue构造函数上的属性，在globalAPI中初始化
